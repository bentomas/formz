...
pub fn parse_single_field_form_test() {
  forma.new()
  |> forma.add(field("first", string_fields.text_field()))
...
pub fn parse_double_field_form_test() {
  forma.new()
  |> forma.add(field("first", string_fields.text_field()))
  |> forma.add(field("second", string_fields.text_field()))
...
pub fn parse_double_field_form_with_error_test() {
  let form =
    forma.new()
    |> forma.add(field("a", string_fields.integer_field()))
    |> forma.add(field("b", string_fields.integer_field()))
  ...
  let assert [fielda, fieldb] = forma.get_inputs(f)
...
pub fn parse_triple_field_form_with_error_test() {
  let form =
    ...
    |> forma.add(field("a", string_fields.integer_field()))
    |> forma.add(field("b", string_fields.integer_field()))
    |> forma.add(field("c", string_fields.integer_field()))
  ...
  let assert Error(f) =
    ...
    |> forma.parse
  let assert [fielda, fieldb, fieldc] = forma.get_inputs(f)
  fielda |> should_be_field_no_error
  fieldb |> should_be_field_no_error
...
pub fn parse_and_try_test() {
  ...
  let assert Error(form) =
    forma.parse_and_try(f, fn(_, form) {
      Error(forma.update_input(form, "a", input.set_error(_, "woops")))
    })
